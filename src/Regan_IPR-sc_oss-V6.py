...
def main():
    ...
    if total_moves_fit < MIN_MOVES:
        ...
        # Calculate AE_e (Expected Average Error/Difference) using the fitted parameters
        ...
        for vals, played_idx in global_test_set:
            ...
            # Calculate deltas in WP space (consistent with modeling)
            ...
            # Expected Error for this position = Sum( prob[i] * delta[i] )
            expected_pos_error = sum(p * d for p, d in zip(probs, deltas))
            
            total_expected_error += expected_pos_error
            total_positions += 1
            
        ae_e = (total_expected_error / total_positions) if total_positions > 0 else 0
        ...
        if success:
            ...
            print(f"  Result: s={s_fit:.4f}, c={c_fit:.4f}, AE_e={ae_e:.6f}")
    ...
    if final_output:
        ...
        fields = ['filename', 'AvgElo', 'MinElo', 'MaxElo', 's', 'c', 'AE_e', 'FirstYear', 'LastYear']
